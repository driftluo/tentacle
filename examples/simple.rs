...
impl ServiceProtocol for PHandle {
    ...
    fn connected(&mut self, mut control: HandleContextMutRef, version: &str) {
    ...
    fn disconnected(&mut self, control: HandleContextMutRef) {
        let new_list = self
            ...
            .iter()
            .filter(|&id| id != &control.session_context.id)
            .cloned()
            .collect();
        self.connected_session_ids = new_list;

        if let Some(handle) = self.clear_handle.remove(&control.session_context.id) {
            let _ = handle.send(());
        }

        info!(
            "proto id [{}] close on session [{}]",
            self.proto_id, control.session_context.id
        );
    }

    fn received(&mut self, env: HandleContextMutRef, data: bytes::Bytes) {
        self.count += 1;
        info!(
            "received from [{}]: proto [{}] data {:?}, message count: {}",
            env.session_context.id,
            self.proto_id,
    ...
    fn notify(&mut self, _control: &mut HandleContext, token: u64) {
