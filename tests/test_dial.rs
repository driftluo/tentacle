...
impl ServiceProtocol for PHandle {
    ...
    fn connected(&mut self, control: HandleContextMutRef, _version: &str) {
        if control.session_context.ty == SessionType::Server {
            // if server, dial itself
            self.dial_addr = Some(control.listens()[0].clone());
        } else {
            // if client, dial server
            self.dial_addr = Some(control.session_context.address.clone());
        }
    ...
    fn notify(&mut self, control: &mut HandleContext, _token: u64) {
